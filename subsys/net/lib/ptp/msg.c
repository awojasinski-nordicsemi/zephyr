...
static void ptp_timestamp_post_recv(struct ptp_msg *msg, struct ptp_protocol_timestamp *ts);
static void ptp_timestamp_pre_send(struct ptp_protocol_timestamp *ts);
static int ptp_msg_header_post_recv(struct ptp_header *msg);
static void ptp_msg_header_pre_send(struct ptp_header *msg);
static void ptp_port_id_post_recv(struct ptp_port_id *port_id);
static void ptp_port_id_pre_send(struct ptp_port_id *port_id);
...
static int ptp_msg_header_post_recv(struct ptp_header *header)
{
...
static void ptp_msg_header_pre_send(struct ptp_header *header)
{
...
int ptp_mgs_pre_send(struct ptp_msg *msg)
{
	enum ptp_msg_type type = ptp_msg_type_get(msg);

	ptp_msg_header_pre_send(&msg->header);
	...
	case PTP_MSG_PDELAY_RESP:
		ptp_port_id_pre_send(&msg->pdelay_resp.req_port_id);
		break;
	case PTP_MSG_FOLLOW_UP:
		break;
	case PTP_MSG_DELAY_RESP:
		ptp_port_id_pre_send(&msg->delay_resp.req_port_id);
		break;
	case PTP_MSG_PDELAY_RESP_FOLLOW_UP:
		ptp_port_id_pre_send(&msg->pdelay_resp_follow_up.req_port_id);
		break;
	case PTP_MSG_ANNOUNCE:
		break;
	case PTP_MSG_SIGNALING:
		ptp_port_id_pre_send(&msg->signaling.target_port_id);
		break;
	case PTP_MSG_MANAGEMENT:
		ptp_port_id_pre_send(&msg->management.target_port_id);
		break;
	}
}

int ptp_mgs_post_recv(struct ptp_msg *msg)
{
	if (ptp_msg_header_post_recv(&msg->header)) {
		return;
	}
	...
	{
	case PTP_MSG_SYNC:
		ptp_timestamp_post_recv(msg, &msg->sync.origin_timestamp);
		break;
	case PTP_MSG_DELAY_REQ:
		break;
	case PTP_MSG_PDELAY_REQ:
		break;
	case PTP_MSG_PDELAY_RESP:
		ptp_timestamp_post_recv(msg, &msg->pdelay_resp.req_receipt_timestamp);
		ptp_port_id_post_recv(&msg->pdelay_resp.req_port_id);
		break;
	case PTP_MSG_FOLLOW_UP:
		ptp_timestamp_post_recv(msg, &msg->follow_up.precise_origin_timestamp);
		break;
	case PTP_MSG_DELAY_RESP:
		ptp_timestamp_post_recv(msg, &msg->delay_resp.receive_timestamp);
		ptp_port_id_post_recv(&msg->delay_resp.req_port_id);
		break;
	case PTP_MSG_PDELAY_RESP_FOLLOW_UP:
		ptp_timestamp_post_recv(msg, &msg->pdelay_resp_follow_up.resp_origin_timestamp);
		ptp_port_id_post_recv(&msg->pdelay_resp_follow_up.req_port_id);
		break;
	case PTP_MSG_ANNOUNCE:
		ptp_timestamp_post_recv(msg, &msg->announce.origin_timestamp);
		break;
	case PTP_MSG_SIGNALING:
		ptp_port_id_post_recv(&msg->signaling.target_port_id);
		break;
	case PTP_MSG_MANAGEMENT:
		ptp_port_id_post_recv(&msg->management.target_port_id);
		break;
	}
}
