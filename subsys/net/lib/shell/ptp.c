...
static void ptp_port_cb(int port, struct net_if *iface, void *user_data)
{
...
#if defined(CONFIG_NET_PTP_STATISTICS)
	PR("\nStatistics:\n");
	PR("Sync %s %s                 : %u\n",
...
static int cmd_net_ptp_port(const struct shell *sh, size_t argc, char *argv[])
{
	...
	PR_INFO("Set CONFIG_NET_PTP to enable PTP support.", "CONFIG_NET_PTP");
#endif
...
static int cmd_net_ptp(const struct shell *sh, size_t argc, char *argv[])
{
	...
	PR_INFO("Set CONFIG_NET_PTP to enable PTP support.\n");
#endif

	return 0;
}

SHELL_STATIC_SUBCMD_SET_CREATE(net_cmd_ptp,
	SHELL_CMD(port, NULL,
		  "'net ptp [<port>]' prints detailed information about PTP port.",
		  cmd_net_ptp_port),
	SHELL_SUBCMD_SET_END
);

SHELL_SUBCMD_ADD((net), ptp, &net_cmd_ptp,
		 "Print information about PTP support.",
		 cmd_net_ptp, 1, 1);
